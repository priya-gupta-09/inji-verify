    ...
    return (
        <nav className="bg-background border-gray-200 xs:px-4 lg:px-20 py-3.5 rounded drop-shadow-md z-25 relative">
            <div className="container flex flex-wrap justify-between items-center h-[40px] mx-0">
                {!props.fromComposableBanking && <button data-collapse-toggle="navbar-default" type="button"
                        className={`${showMenu?`bg-${window._env_.DEFAULT_THEME}-lighter-gradient`:"bg-background"} inline-flex items-center mr-1 p-3 text-sm text-gray-500 rounded-md lg:hidden dark:text-gray-400`}
                ...
                <a href={Pages.Home} className="flex items-center">
                    <Logo className={!props.fromComposableBanking ? "w-20 lg:w-[184px] scale-150 cursor-pointer" : "ml-10 w-20 lg:w-[184px] scale-150 cursor-pointer"}/>
                </a>
                {!props.fromComposableBanking && <DesktopMenu/> }
