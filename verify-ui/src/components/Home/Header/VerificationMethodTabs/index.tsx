...
const Tab = ({
  ...
  const activeTab = `bg-${window._env_.DEFAULT_THEME}-gradient border-t-[6px] border-y-transparent text-activeTabText`;
...
function VerificationMethodTabs(props: any) {
  ...
  function showAlert() {
    dispatch(
      raiseAlert({
        ...AlertMessages().verificationMethodComingSoon,
        open: true,
      })
    );
  }
  ...
  return (
    <div
      className={`container mx-auto w-[100%] bg-${window._env_.DEFAULT_THEME}-lighter-gradient  max-w-[100vw] overflow-x-hidden lg:overflow-x-auto`}
    >
